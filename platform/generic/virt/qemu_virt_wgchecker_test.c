					...
					const struct qemu_virt_wg_checker_expect *expect)
{
	...
	SBIUNIT_ASSERT_EQ(test,
			  ...
			  expect->slot_count);
	SBIUNIT_ASSERT_EQ(test,
			  qemu_virt_wg_mmio_read64(base +
						  WGCHECKER2_MMIO_ERRCAUSE),
			  0UL);
	SBIUNIT_ASSERT_EQ(test,
			  qemu_virt_wg_mmio_read64(base +
						  WGCHECKER2_MMIO_ERRADDR),
			  0UL);

	for (slot = 1; slot < expect->slot_count; slot++) {
		addr = qemu_virt_wg_mmio_read64(base +
						WGCHECKER2_MMIO_SLOT_BASE +
						slot * WGCHECKER2_MMIO_SLOT_STRIDE +
						WGCHECKER2_MMIO_SLOT_ADDR);
		perm = qemu_virt_wg_mmio_read64(base +
						WGCHECKER2_MMIO_SLOT_BASE +
						slot * WGCHECKER2_MMIO_SLOT_STRIDE +
	...
	perm = qemu_virt_wg_mmio_read64(base + WGCHECKER2_MMIO_SLOT_BASE +
					expect->slot_count *
						WGCHECKER2_MMIO_SLOT_STRIDE +
					WGCHECKER2_MMIO_SLOT_PERM);
	cfg = readl((void *)(unsigned long)
		    (base + WGCHECKER2_MMIO_SLOT_BASE +
...
static void qemu_virt_wg_boot_test(struct sbiunit_test_case *test)
{
	...
	rc = worldguard_test_check_domain_state(dom0, true,
						qemu_virt_wg_domain0_expect.wid,
						qemu_virt_wg_domain0_expect.widlist_mask);
	SBIUNIT_ASSERT_EQ(test, rc, 0);
	rc = worldguard_test_check_domain_state(dom1, true,
						qemu_virt_wg_domain1_expect.wid,
...
const struct sbi_hart_protection_test_ops qemu_virt_worldguard_test_ops = {
