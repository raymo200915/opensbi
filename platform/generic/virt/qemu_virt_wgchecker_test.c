...
static struct qemu_virt_wg_slot_expect qemu_virt_wg_dram_slots[] = {
	{ .check_addr = true, .addr = 0x20000000ULL, .perm = 0, .cfg = QEMU_VIRT_WG_SLOT_CFG_A_OFF },
	{ .check_addr = true, .addr = 0x30000000ULL, .perm = 0xcf, .cfg = QEMU_VIRT_WG_SLOT_CFG_A_TOR },
	{ .check_addr = true, .addr = 0x30400000ULL, .perm = 0xcc, .cfg = QEMU_VIRT_WG_SLOT_CFG_A_TOR },
	{ .check_addr = true, .addr = 0x40000000ULL, .perm = 0xcf, .cfg = QEMU_VIRT_WG_SLOT_CFG_A_TOR },
...
static const struct qemu_virt_wg_checker_expect qemu_virt_wg_checker_expects[] = {
	...
	},
	{
		...
		.last_slot = { .perm = 0xc3, .cfg = QEMU_VIRT_WG_SLOT_CFG_A_TOR },
	},
	{
		.path = "/soc/wgchecker@6002000",
		.slot_count = 1,
		.slots = NULL,
		.last_slot = { .perm = 0xc0, .cfg = QEMU_VIRT_WG_SLOT_CFG_A_TOR },
	},
};
					...
					const struct qemu_virt_wg_checker_expect *expect)
{
	...
	SBIUNIT_ASSERT_EQ(test,
			  readl((void *)(unsigned long)
				(base + QEMU_VIRT_WG_MMIO_NSLOTS)),
			  expect->slot_count);
	SBIUNIT_ASSERT_EQ(test,
			  qemu_virt_wg_mmio_read64(base + QEMU_VIRT_WG_MMIO_ERRCAUSE),
			  0UL);
	SBIUNIT_ASSERT_EQ(test,
			  qemu_virt_wg_mmio_read64(base + QEMU_VIRT_WG_MMIO_ERRADDR),
			  0UL);

	for (slot = 1; slot < expect->slot_count; slot++) {
		addr = qemu_virt_wg_mmio_read64(
			base + QEMU_VIRT_WG_MMIO_SLOT_BASE +
	...
	perm = qemu_virt_wg_mmio_read64(
		base + QEMU_VIRT_WG_MMIO_SLOT_BASE +
		expect->slot_count * QEMU_VIRT_WG_MMIO_SLOT_STRIDE +
		QEMU_VIRT_WG_MMIO_SLOT_PERM);
	cfg = readl((void *)(unsigned long)
		    (base + QEMU_VIRT_WG_MMIO_SLOT_BASE +
		     expect->slot_count * QEMU_VIRT_WG_MMIO_SLOT_STRIDE +
...
static void qemu_virt_wg_boot_test(struct sbiunit_test_case *test)
{
	...
	SBIUNIT_ASSERT_EQ(test,
			  qemu_virt_wgchecker_test_check_platform_state(3, true),
			  0);
	...
	SBIUNIT_ASSERT_EQ(test,
			  qemu_virt_wgchecker_test_check_domain_state(
...
const struct sbi_hwiso_test_ops qemu_virt_wgchecker_test_ops = {
