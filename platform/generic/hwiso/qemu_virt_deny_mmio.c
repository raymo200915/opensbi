				   ...
				   const struct sbi_domain *dst, void *ctx)
{
	...
	if (scratch->options & SBI_SCRATCH_DEBUG_PRINTS)
		sbi_printf("qemu-virt-hwiso: cleared deny list\n");
}
				    ...
				    const struct sbi_domain *src, void *ctx)
{
	...
	if ((scratch->options & SBI_SCRATCH_DEBUG_PRINTS) && state->count)
		sbi_printf("qemu-virt-hwiso: deny list applied (%u entries)\n",
			   state->count);
}
