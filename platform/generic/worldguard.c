...
struct worldguard_find_domain_info {
	const char *name;
	int domain_offset;
};
...
static struct sbi_domain_data worldguard_domain_data = {
	.data_size = sizeof(struct worldguard_domain_state),
	.data_setup = worldguard_domain_data_setup,
	.data_cleanup = worldguard_domain_data_cleanup,
};
...
static int worldguard_parse_wid_prop(const void *fdt, int node,
				     const char *prop_name, u32 *out_wid)
{
	const fdt32_t *prop;
	int len;
...
static int worldguard_parse_widlist(const void *fdt, int node,
				    const char *prop_name, u32 *out_mask)
{
	const fdt32_t *prop;
...
static int worldguard_find_domain_offset_match(void *fdt, int domain_offset,
					       void *opaque)
{
	struct worldguard_find_domain_info *info = opaque;
	const char *name;

	name = fdt_get_name(fdt, domain_offset, NULL);
	if (name && !sbi_strcmp(info->name, name)) {
...
static int worldguard_find_domain_offset(const void *fdt,
					 const struct sbi_domain *dom)
{
	struct worldguard_find_domain_info info;
	int rc;
	...
	rc = fdt_iterate_each_domain((void *)fdt, &info,
				     worldguard_find_domain_offset_match);
	if (rc < 0)
		return rc;
					 ...
					 struct worldguard_domain_state *state)
{
	...
	domain_offset = worldguard_find_domain_offset(fdt, dom);
	if (domain_offset < 0)
