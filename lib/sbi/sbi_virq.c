...
static inline void virq_set_deferred_notify(struct sbi_domain *dom,
					    ...
					    u32 deferred_virq)
{
	...
	spin_lock(&st->lock);
	st->deferred_notify = deferred_notify;
	st->deferred_virq = deferred_virq;
	spin_unlock(&st->lock);
}

static int notify_smode_thishart(void)
{
	int ret;
...
void sbi_virq_return_to_prev_if_needed(void)
{
	...
	sbi_printf("[domain] return-to-prev after SSE complete on hart%lu\n",
		   (unsigned long)current_hartindex());
	rc = sbi_domain_context_exit_to_prev();
	if (rc && rc != SBI_ENOENT)
		sbi_printf("[domain] return-to-prev failed, rc=%d\n", rc);
}
...
void sbi_virq_notify_smode_deferred_thishart(void)
{
	...
	spin_lock(&st->lock);
	if (!st->deferred_notify) {
		spin_unlock(&st->lock);
		return;
	}
	spin_unlock(&st->lock);

	rc = sbi_sse_inject_event(event);
	sbi_printf("[VIRQ] Inject deferred SSE event 0x%lx, ret:%d\n",
		   event, rc);
	if (!rc) {
		spin_lock(&st->lock);
		st->deferred_notify = false;
		spin_unlock(&st->lock);
		sbi_printf("[VIRQ] Deferred SSE injected\n");
	} else {
		sbi_printf("[VIRQ] Inject deferred SSE event failed rc=%d\n",
			   rc);
	}
}
...
int sbi_virq_courier_handler(u32 hwirq, void *opaque)
{
	...
	/* Route purely by HWIRQ -> Domain rules (from FDT). */
	dom = sbi_virq_route_lookup_domain(hwirq);
	if (!dom)
		return SBI_EINVAL;

	sbi_printf("[domain] virq courier hart%lu curr=%s target=%s hwirq=%u\n",
		   (unsigned long)current_hartindex(),
	...
	sbi_printf("[VIRQ] route hwirq %u, chip_uid %u -> dom (%s), VIRQ %u\n",
		   hwirq, ctx->chip->id, dom->name, virq);
	...
	/* Switch to target domain context if needed */
	if (dom != sbi_domain_thishart_ptr()) {
		/*
		 * Mark return-to-prev and request deferred notify so that
		 * first-entry can still receive SSE after it unmasks.
		 */
		virq_set_domain_return_flag(dom, true);
		virq_set_deferred_notify(dom, true, virq);
		sbi_printf("[domain] virq courier switching hart%lu %s -> %s\n",
			   (unsigned long)current_hartindex(),
			   sbi_domain_thishart_ptr()->name, dom->name);
		rc = sbi_domain_context_enter(dom);
		if (rc) {
			virq_set_domain_return_flag(dom, false);
			virq_set_deferred_notify(dom, false, 0);
			/* switch failed; re-enable to avoid deadlock */
			sbi_irqchip_unmask_hwirq(ctx->chip, hwirq);
			return rc;
		}
	}

	rc = notify_smode_thishart();
	if (rc) {
		if (rc == SBI_EINVALID_STATE) {
			/* keep hwirq masked; retry after SSE unmask */
			return SBI_OK;
		}
...
int sbi_virq_domain_init(struct sbi_domain *dom)
{
	...
	for (i = 0; i < nharts; i++) {
		...
		p->st[i].head = 0;
		p->st[i].tail = 0;
		p->st[i].deferred_notify = false;
		p->st[i].deferred_virq = 0;
		p->st[i].return_to_prev = false;
	}
	dom->virq_priv = p;
