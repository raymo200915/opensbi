...
static struct sbi_domain **g_virq_pending_dom;
static spinlock_t g_virq_pending_lock;

static void virq_pending_set(u32 hartindex, struct sbi_domain *dom)
{
	spin_lock(&g_virq_pending_lock);
	g_virq_pending_dom[hartindex] = dom;
	spin_unlock(&g_virq_pending_lock);
}

static struct sbi_domain *virq_pending_get_clear(u32 hartindex)
{
	struct sbi_domain *dom;

	spin_lock(&g_virq_pending_lock);
	dom = g_virq_pending_dom[hartindex];
	g_virq_pending_dom[hartindex] = NULL;
	spin_unlock(&g_virq_pending_lock);
...
u32 sbi_virq_pop_thishart(void)
{
	...
	pending_dom = virq_pending_get_clear(hartindex);
	if (pending_dom && pending_dom != dom) {
		/* Mark return_to_prev for VIRQ-driven domain switch. */
		virq_set_domain_return_flag(pending_dom, true);
		sbi_printf("[VIRQ] virq pop switching hart%u %s -> %s\n",
			   hartindex, dom->name, pending_dom->name);
		rc = sbi_domain_context_enter(pending_dom);
		if (!rc)
			return sbi_virq_pop_thishart();
	}
...
void sbi_virq_return_to_prev_if_needed(void)
{
	...
	sbi_printf("[VIRQ] return_to_prev after VIRQ complete on hart%lu\n",
		   (unsigned long)current_hartindex());
	sbi_printf("[dbg] return_to_prev: mstatus=0x%lx mepc=0x%lx sepc=0x%lx\n",
		   csr_read(CSR_MSTATUS), csr_read(CSR_MEPC),
		   csr_read(CSR_SEPC));
	/*
	 * If we are returning from an S-mode trap (ecall), MEPC is the S-mode
	 * return point. Sync it into SEPC so the previous domain resumes
	 * correctly after the context switch.
	 */
	if (((csr_read(CSR_MSTATUS) & MSTATUS_MPP) >> MSTATUS_MPP_SHIFT) ==
	    PRV_S)
		csr_write(CSR_SEPC, csr_read(CSR_MEPC));
	rc = sbi_domain_context_exit_to_prev();
	if (rc && rc != SBI_ENOENT)
		sbi_printf("[VIRQ] return_to_prev failed, rc=%d\n", rc);
}
...
int sbi_virq_courier_handler(u32 hwirq, void *opaque)
{
	...
	sbi_printf("[VIRQ] virq courier hart%lu curr=%s target=%s hwirq=%u\n",
		   (unsigned long)current_hartindex(),
		   sbi_domain_thishart_ptr()->name, dom->name, hwirq);
	...
	sbi_printf("[VIRQ] route hwirq %u, chip_uid %u -> dom (%s), channel %u, VIRQ %u\n",
		   hwirq, ctx->chip->id, dom->name, channel_id, virq);
	...
	/*
	 * Notify S-mode before any domain switch so first-entry domains can
	 * observe the pending notification after enabling SEIE.
	 */
	...
	/* Ensure EOI before any domain switch so raw handler can complete. */
	...
	/*
	 * Do not switch domains in M-mode IRQ context. Let S-mode ecall
	 * (VIRQ_POP) trigger the domain switch.
	 */
	if (dom != sbi_domain_thishart_ptr())
		virq_pending_set(current_hartindex(), dom);

	return eoi_done ? SBI_EALREADY : SBI_OK;
}
...
int sbi_virq_init(u32 init_virq_cap)
{
	...
	SPIN_LOCK_INIT(g_virq_pending_lock);
	g_virq_pending_dom = sbi_zalloc(sizeof(*g_virq_pending_dom) *
					sbi_hart_count());
	if (!g_virq_pending_dom)
		return SBI_ENOMEM;
