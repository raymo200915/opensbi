					  ...
					  struct hart_context *dom_ctx)
{
	...
	sbi_printf("[domain] restored hart%u target=%s mepc=0x%lx "
		   "mstatus=0x%lx sstatus=0x%lx sie=0x%lx sip=0x%lx "
		   "mip=0x%lx mie=0x%lx mideleg=0x%lx\n",
		   hartindex, target_dom->name, trap_ctx->regs.mepc,
		   trap_ctx->regs.mstatus, csr_read(CSR_SSTATUS),
		   csr_read(CSR_SIE), csr_read(CSR_SIP),
