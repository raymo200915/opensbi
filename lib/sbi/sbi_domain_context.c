					  ...
					  struct hart_context *dom_ctx)
{
	...
	sbi_printf("[dbg] save %s: mstatus=0x%lx mepc=0x%lx sepc=0x%lx\n",
		   current_dom->name, csr_read(CSR_MSTATUS),
		   csr_read(CSR_MEPC), csr_read(CSR_SEPC));
	 ...
	 * point. In that case, override the saved SEPC so returning to this
	 * domain resumes correctly after a VIRQ-driven domain switch.
	 */
	if (((csr_read(CSR_MSTATUS) & MSTATUS_MPP) >> MSTATUS_MPP_SHIFT) ==
	    PRV_S)
		ctx->sepc = csr_read(CSR_MEPC);
