...
/* Map MPXY MM channel to MPXY ReqFwd channel. TODO: Map by DT property */
static int mm_channel_mapping(u32 mm_channel_id, u32 *map_channel_id)
{
	switch (mm_channel_id) {
	case 0:
		/* MM channel 0 is mapped to ReqFwd channel 16 */
		*map_channel_id = 16;
		break;
	case 1:
		/* MM channel 1 is mapped to ReqFwd channel 17 */
		*map_channel_id = 17;
		break;
	default:
		return SBI_ENOTSUPP;
					 ...
					 unsigned long *ack_len)
{
	...
	if (RPMI_MM_SRV_COMMUNICATE == message_id) {
		rc = mm_channel_mapping(channel->channel_id, &recv_channel_id);
		if (rc)
			return rc;
		...
		sbi_domain_context_enter(mm->tdomain);
	} else {
		return SBI_EFAIL;
	}
