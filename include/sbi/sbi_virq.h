...
/*
 * Delay before returning to the previous domain after a VIRQ has completed.
 * This is intentionally timer based for stress-testing asynchronous return
 * context switching.
 */
#define VIRQ_RETURN_TO_PREV_DELAY_MS 1
...
struct sbi_domain_virq_state {
	...
	/* M-mode return timer scheduled for this hart/domain context. */
	bool return_timer_pending;
};
...
/* Return to previous domain from M-mode timer context. */
void sbi_virq_return_to_prev_from_timer(void);

/* Check whether a VIRQ return timer is pending on this hart. */
